...
```{r}
tar_load(dino)
tar_load(h04_q02_m_exp_brms_sample)

plot(conditional_effects(h04_q02_m_exp_brms_sample), points = TRUE)

newdata <- dino[, data_grid(.SD, scale_age = seq_range(scale_age, n = 10)),
								by = species_factor]
...
newdata <- dino[, data_grid(.SD, scale_age = seq_range(scale_age, n = 10)),
								by = species_factor]
epred <- h04_q02_m_lin_brms_sample |> 
    epred_draws(newdata)
