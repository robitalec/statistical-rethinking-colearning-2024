...
marg_eff <- emmeans(h05_q01_brms_sample, ~gender)
marg_eff
contrast(marg_eff)
```
...
q03_pred$Handedness <- c('R', 'L')[grepl('L-', q03_pred$hand_pair) + 1]
ggplot(q03_pred) + 
	stat_halfeye(aes(.prediction / n_fight, hand_pair), alpha = 0.5) +
	labs(x = 'Predicted win rate', y = 'Hand pair') + 
	facet_wrap(~Handedness, ncol  = 1, scales = 'free_y') +
	labs(y = '') + 
	xlim(0, 1)
